defmodule ElixirGist.GistsTest do
  ...
  describe "gists" do
    ...
    @invalid_attrs %{name: nil, descripcion: nil, markup_text: nil}
    ...
    test "create_gist/1 with valid data creates a gist" do
      valid_attrs = %{name: "some name", descripcion: "some descripcion", markup_text: "some markup_text"}

      assert {:ok, %Gist{} = gist} = Gists.create_gist(valid_attrs)
      assert gist.name == "some name"
      assert gist.descripcion == "some descripcion"
    ...
    test "update_gist/2 with valid data updates the gist" do
      gist = gist_fixture()
      update_attrs = %{name: "some updated name", descripcion: "some updated descripcion", markup_text: "some updated markup_text"}

      assert {:ok, %Gist{} = gist} = Gists.update_gist(gist, update_attrs)
      assert gist.name == "some updated name"
      assert gist.descripcion == "some updated descripcion"
