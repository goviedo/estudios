...
async function main() {
  ...
  const marketsentimentedAmount = hre.ethers.utils.parseEther("1");

  const MarketSentiment = await hre.ethers.getContractFactory(
    "MarketSentiment"
  );
  const marketsentiment = await MarketSentiment.deploy(unmarketsentimentTime, {
    value: marketsentimentedAmount,
  });
